<?php

namespace App\Http\Controllers\Staff;

use App\Http\Controllers\Controller;
use App\Models\EmpresaHorario;
use Illuminate\Http\RedirectResponse;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;
use Illuminate\View\View;

class EmpresaHorarioController extends Controller
{
    private array $dias = [
        1 => 'Lunes', 2 => 'Martes', 3 => 'Miércoles', 4 => 'Jueves',
        5 => 'Viernes', 6 => 'Sábado', 0 => 'Domingo',
    ];

    public function edit(): View
    {
        $empresa = Auth::guard('web')->user()->empresa;

        $horarios = $empresa->horarios()->orderBy('dia_semana')->orderBy('hora_inicio')->get();

        return view('staff.empresa-horarios', [
            'horarios' => $horarios,
            'dias' => $this->dias,
        ]);
    }

    public function store(Request $request): RedirectResponse
    {
        $empresa = Auth::guard('web')->user()->empresa;

        $data = $request->validate([
            'dias' => ['required', 'array', 'min:1'],
            'dias.*' => ['integer', 'between:0,6'],
            'hora_inicio' => ['required', 'date_format:H:i'],
            'hora_fin' => ['required', 'date_format:H:i', 'after:hora_inicio'],
        ]);

        $creados = [];
        $conflictos = [];

        foreach ($data['dias'] as $dia) {
            $solapa = $empresa->horarios()
                ->where('dia_semana', $dia)
                ->where(function ($q) use ($data) {
                    $q->where('hora_inicio', '<', $data['hora_fin'])
                        ->where('hora_fin', '>', $data['hora_inicio']);
                })
                ->exists();

            if ($solapa) {
                $conflictos[] = $this->dias[$dia];
                continue;
            }

            $empresa->horarios()->create([
                'dia_semana' => $dia,
                'hora_inicio' => $data['hora_inicio'],
                'hora_fin' => $data['hora_fin'],
            ]);

            $creados[] = $this->dias[$dia];
        }

        $mensaje = $creados
            ? 'Horario agregado para: ' . implode(', ', $creados) . '.'
            : 'No se agregó ningún horario.';

        if ($conflictos) {
            $mensaje .= ' Se omitió por superposición: ' . implode(', ', $conflictos) . '.';
        }

        return back()->with('status', $mensaje);
    }

    public function destroy(EmpresaHorario $horario): RedirectResponse
    {
        abort_if($horario->empresa_id !== Auth::guard('web')->user()->empresa_id, 403);

        $horario->delete();

        return back()->with('status', 'Horario eliminado.');
    }
}