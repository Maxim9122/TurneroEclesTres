<x-layouts.app title="Horarios - EclesTres" :logout-route="route('staff.logout')">
    <div class="flex items-center justify-between mb-6">
        <h1 class="font-display text-2xl">Horarios de {{ $profesional->nombre }}</h1>
        @if ($profesional->usaHorarioGeneral())
            <div class="mb-4 text-sm text-mate-tinta/70 bg-mate-borde/30 border border-mate-borde rounded-md p-3">
                Este profesional todavía no tiene un horario propio cargado, así que está usando el
                <a href="{{ auth('web')->user()->esAdmin() ? route('staff.empresa.horario-general.edit') : '#' }}" class="underline">
                    horario general del negocio
                </a>.
                Si cargás un horario acá abajo, este profesional pasa a usar SU PROPIO horario en vez del general.
            </div>
        @endif
        <a href="{{ auth('web')->user()->esAdmin() ? route('staff.empresa.profesionales.index') : route('staff.empresa.dashboard') }}"
            ...
            <div class="bg-mate-superficie border border-mate-borde rounded-lg p-4">
                ...
                @forelse ($horariosDia as $horario)
                    <div class="flex items-center justify-between text-sm py-1">
                        ...
                        <form method="POST" action="{{ route('staff.empresa.profesionales.horarios.destroy', [$profesional, $horario]) }}">
                            @csrf
                            @method('DELETE')
                ...
                @empty
                    <p class="text-xs text-mate-tinta/50">No atiende este día.</p>
                @endforelse
            </div>
        @endforeach
    </div>
    ...
    <div class="bg-mate-superficie border border-mate-borde rounded-lg p-5 max-w-md">
        ...
        <form method="POST" action="{{ route('staff.empresa.profesionales.horarios.store', $profesional) }}" class="space-y-3">
            @csrf
            ...
            <p class="text-xs text-mate-tinta/50">
                Se va a cargar el mismo rango horario para todos los días que marques. Si atiende con corte al mediodía, cargá primero el rango de la mañana y después agregá el de la tarde marcando los mismos días.
            </p>
