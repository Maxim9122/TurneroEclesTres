<x-layouts.app title="Mi perfil - EclesTres" :logout-route="route('cliente.logout')">
    ...
    <div class="grid gap-6 max-w-md">
        <form method="POST" action="{{ route('cliente.mi-perfil.update') }}"
            ...
            <div>
                ...
                <input type="text" name="nombre" required value="{{ old('nombre', auth('cliente')->user()->nombre) }}"
            ...
            <div>
                ...
                <input type="email" name="email" required value="{{ old('email', auth('cliente')->user()->email) }}"
                    class="w-full rounded-md border border-mate-borde bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-mate-salvia">
            </div>

            <div>
                <label class="block text-sm mb-1">Teléfono (WhatsApp)</label>
                <input type="tel" name="telefono" value="{{ old('telefono', auth('cliente')->user()->telefono) }}"
                    placeholder="Ej: 3841670079"
                    class="w-full rounded-md border border-mate-borde bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-mate-salvia">
                <p class="text-xs text-mate-tinta/50 mt-1">Lo usamos para enviarte confirmaciones y comprobantes por WhatsApp.</p>
            </div>
        ...
        <form method="POST" action="{{ route('cliente.mi-perfil.password') }}"
            class="bg-mate-superficie border border-mate-borde rounded-lg p-5 space-y-4">
            @csrf
            @method('PUT')
